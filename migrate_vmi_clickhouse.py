...
def migrate_data(year=None, week=None, company=None):
    ...
    with maria_conn.cursor() as cursor:
        ...
        cursor.execute(f"SELECT COUNT(*) AS cnt FROM tbl_vmi_pre_aggregated_data {where_clause}", params)
        total_rows = cursor.fetchone()["cnt"]
        print(f" Starting migration of {total_rows} rows...")

        while True:
            query = f"SELECT * FROM tbl_vmi_pre_aggregated_data {where_clause} LIMIT %s OFFSET %s"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Migrate VMI data from MariaDB to ClickHouse")
    parser.add_argument("--year", type=int, help="Filter by year")
    parser.add_argument("--week", type=int, help="Filter by week")
    parser.add_argument("--company", type=int, help="Filter by company")
    args = parser.parse_args()

    migrate_data(year=args.year, week=args.week, company=args.company)
