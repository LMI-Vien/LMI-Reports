...
class StoreSalesPerfPerArea extends BaseController
{
	...
	private function printFilter($pdf, $filters) {
		...
		// code explanation at "StoreSalesPerfPerBa"
		$lineHeight = 6; 
		...
		foreach ($rows as $rowFilters) {
			...
			// PARAMETERS IS EXPLAINED AT "StoreSalesPerfPerBa"
			foreach ($rowFilters as $key => $value) {
				$pdf->MultiCell($colWidth, $lineHeight, "<b>{$key}:</b> {$value}", 0, 'L', 0, 0, '', '', true, 0, true);
			}
		...
		$pdf->Cell(85, 6, '', 0, 0); // spacer for the current week and gen date
		$pdf->writeHTMLCell(
			0, 6, '', '',
	...
	public function generatePdf() {	
		...
		foreach ($headers as $i => $text) {
			if ($text === 'Target Per Remaining Days') {
