        ...
        <li class="nav-item filter-hover">
          <div class="filter-content">
            <div>
              ...
              <div class="form-group card-dark mb-3 p-2">
                  <label for="itmCode" class="mb-2"><i class="fas fa-briefcase mr-1"></i> Item Code</label>
                  <select id="itmCode" name="itmCode[]" class="form-control select2" multiple>
                      <?php foreach ($sku_item as $key => $value) {
                          echo '<option value="' . htmlspecialchars($value['item_code']) . '">' . htmlspecialchars($value['item_code']) . '</option>';
                      } ?>
                  </select>
              </div>
