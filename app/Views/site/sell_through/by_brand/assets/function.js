    ...
    function fetchData() {
        ...
        if ($.fn.DataTable.isDataTable('#sellThroughByBrand')) {
            let existingTable = $('#sellThroughByBrand').DataTable();
            existingTable.clear().destroy();
        }

        let table = $('#sellThroughByBrand').DataTable({
            paging: true,
            searching: false,
            ordering: true,
            info: true,
            ...
            colReorder: true, 
            ajax: {
                url: base_url + 'sell-through/get-by-brand',
                type: 'POST',
                data: function(d) {
    ...
    function renderSellThroughChart(page = 1) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                let labels = response.data.map(item => item.brand);
    ...
    function handleAction(action) {
        ...
        $.ajax({
            ...
            success: function(blob, status, xhr) {
                ...
                const filename = rawName
                    || (action === 'exportPdf'
                        ? 'Sell Through by Brand.pdf'
                        : 'Sell Through by Brand.xlsx');
        ...
        `;
        logActivity('Sell Through By Brand', action === 'exportPdf' ? 'Export PDF' : 'Export Excel', remarks, '-', null, null)
